...
@router.get("/download_reports_netzbetreiber", status_code=status.HTTP_200_OK)
async def download_reports(db: AsyncSession = Depends(get_db_async),
                           current_user: models.Nutzer = Depends(oauth.get_current_user)):
    await check_netzbetreiber_role(current_user, "GET", "/download_reports_netzbetreiber")
    try:
        # Anzahl der Tarife
        tarife_count = await db.execute(select(func.count(models.Tarif.tarif_id)))

        # Anzahl der Preisstrukturen
        preisstrukturen_count = await db.execute(select(func.count(models.Preisstrukturen.preis_id)))

        # Anzahl der Haushalte, die einen Vertrag bei dem Netzbetreiber haben
        vertraege_count = await db.execute(
            select(func.count(models.Vertrag.vertrag_id)).where(models.Vertrag.netzbetreiber_id == current_user.user_id)
        )
        ...
        # Schreiben der Daten
        writer.writerow(["Anzahl der Tarife", tarife_count.scalar()])
        writer.writerow(["Anzahl der Preisstrukturen", preisstrukturen_count.scalar()])
        writer.writerow(["Anzahl der Haushalte mit Vertrag", vertraege_count.scalar()])

        output.seek(0)  # Zurück zum Anfang der Datei
        return StreamingResponse(io.BytesIO(output.getvalue().encode()), media_type="text/csv", headers={"Content-Disposition": "attachment; filename=netzbetreiber_report.csv"})
